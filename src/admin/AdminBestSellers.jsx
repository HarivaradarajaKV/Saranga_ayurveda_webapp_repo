...
import { Search, Star, Save, RotateCcw, Loader2, ArrowLeft } from 'lucide-react';
import './Admin.css';
...
export default function AdminBestSellers() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.post('/admin/best-sellers', {
    ...
    <div className="page-fade-in">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-light)', marginTop: '2px' }}>
              Select which products appear under the "Best Seller" section on the storefront homepage.
            </p>
          </div>
        </div>
      ...
      {error && (
        <div style={{ padding: '16px 20px', backgroundColor: '#fef2f2', border: '1px solid #fee2e2', borderRadius: '12px', color: '#b91c1c', marginBottom: '24px', display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.9rem' }}>
          ...
          <div>{error}</div>
          <button onClick={fetchData} className="btn btn-secondary" style={{ width: 'max-content', marginTop: '6px', padding: '4px 12px', height: 'auto', fontSize: '0.8rem', color: '#b91c1c', borderColor: '#fca5a5' }}>
            Retry Connection
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', background: 'var(--surface)', padding: '4px', borderRadius: '8px', width: 'max-content', gap: '4px', marginBottom: '16px' }}>
        <button
          ...
          style={{ padding: '6px 16px', borderRadius: '6px', border: 'none', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', backgroundColor: viewMode === 'all' ? 'var(--primary)' : 'transparent', color: viewMode === 'all' ? '#fff' : 'var(--text-light)', transition: 'all 0.2s' }}
        >
          All Products ({products.length})
        </button>
        <button
          onClick={() => setViewMode('selected')}
          style={{ padding: '6px 16px', borderRadius: '6px', border: 'none', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', backgroundColor: viewMode === 'selected' ? 'var(--primary)' : 'transparent', color: viewMode === 'selected' ? '#fff' : 'var(--text-light)', transition: 'all 0.2s' }}
        >
          Selected Best Sellers ({selectedIds.size})
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="admin-table-wrap" style={{ padding: '48px', textAlign: 'center', color: 'var(--text-light)' }}>
          {viewMode === 'selected' ? 'No products have been flagged as Best Sellers yet.' : 'No products match your search or filter criteria.'}
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
          {filteredProducts.map(p => {
            ...
            return (
              <div
                ...
                style={{ padding: '16px', borderRadius: '12px', position: 'relative', cursor: 'pointer', border: isSelected ? '2px solid var(--primary)' : '1px solid var(--border-light)', boxShadow: isSelected ? 'var(--shadow)' : 'var(--shadow-sm)', backgroundColor: isSelected ? 'var(--primary-pale)' : 'var(--bg)' }}
                onClick={() => handleToggle(p.id)}
              >
                <div style={{ width: '100%', aspectRatio: '1 / 1', borderRadius: '8px', overflow: 'hidden', background: '#ffffff', border: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '12px' }}>
                  {p.image_url ? (
                    <img src={getImageUrl(p.image_url)} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '4px' }} onError={(e) => { e.target.src = '/images/placeholder.png'; }} />
                  ) : (
                    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-light)' }}>No Image</div>
                  )}
                </div>
                <h4 style={{ fontSize: '0.88rem', fontWeight: 700, color: 'var(--text)', width: '100%', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '4px' }}>{p.name}</h4>
                <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center', marginTop: '6px' }}>
                  <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--primary)' }}>₹{p.price ? parseFloat(p.price).toFixed(0) : '0'}</span>
                  <div style={{ width: '36px', height: '20px', borderRadius: '100px', backgroundColor: isSelected ? 'var(--primary)' : '#cbd5e1', position: 'relative', transition: 'background-color 0.2s' }}>
                    <div style={{ width: '14px', height: '14px', borderRadius: '50%', backgroundColor: '#fff', position: 'absolute', top: '3px', left: isSelected ? '19px' : '3px', transition: 'left 0.2s' }} />
                  </div>
                </div>
              </div>
            );
