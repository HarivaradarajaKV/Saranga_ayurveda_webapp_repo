...
export default function Home() {
  ...
  useEffect(() => {
    if (location.state?.scrollToFooter) {
      // Clear the state so it doesn't trigger scroll again on page reload
  ...
  const fetchHomeData = async () => {
    setLoading(true);
    try {
      // 1. Nature's Wellness Collection (general first 8 products)
      ...
      // 2. New Arrivals (all available)
      const newRes = await api.get(`${ENDPOINTS.PRODUCTS}?new_arrivals=true`);
      if (newRes.data) {
        setNewArrivals(Array.isArray(newRes.data) ? newRes.data : (newRes.data.products || []));
      }

      // 3. Best Sellers (all available)
      const bestRes = await api.get(`${ENDPOINTS.PRODUCTS}?sort=rating&order=desc`);
      if (bestRes.data) {
        setBestSellers(Array.isArray(bestRes.data) ? bestRes.data : (bestRes.data.products || []));
      }

      // 4. Combo Offers (all available)
      const comboRes = await api.get(ENDPOINTS.COMBOS);
      if (comboRes.data) {
  ...
  return (
    <div className="home page-fade-in">
      ...
      <section className="new-arrivals-section" style={{ paddingTop: 0 }}>
        <div className="container-large">
          <div className="section-header-centered">
            <h2 className="section-title-flat">Best Sellers</h2>
            <p className="section-desc-ayur">
              Our most loved and highly rated natural creations, proven to restore vitality and deliver holistic wellness.
            </p>
          </div>
                ...
                return (
                  <Link to={`/deals/combo/${combo.id}`} key={combo.id} className="new-arrival-card">
                    ...
                    <div className="new-arrival-img-wrap">
                      ...
                      {/* Decorative Capsules near bottom-right of image */}
                      <div className="new-arrival-capsules-dec">
                        <span className="dec-capsule capsule-1" />
                        <span className="dec-capsule capsule-2" />
                      </div>
